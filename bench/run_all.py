...
IMPLS = [
    ...
    dict(key="cell_ia",       label="Cell_IA optimizado (Agents.jl + FFTW)", device="CPU",
         cmd=["julia", "-t1", os.path.join(HERE, "lenia_cellia.jl")], env=CPU_ENV),
    dict(key="cell_ia_legacy", label="Cell_IA legacy (sin optimizar)", device="CPU",
         cmd=["julia", "-t1", os.path.join(HERE, "lenia_cellia.jl")],
         env={**CPU_ENV, "BENCH_CELLIA_STEP": "lenia_model_step_legacy!"}),
